...
- Fail-closed final release tooling and workflow prepared.

## Exact blocker

No approved canonical cover exists at `books/book-05/cover.jpeg`.
...
Supply and explicitly approve a JPEG, RGB, 1,600 × 2,560 px, under-50-MB ebook cover at `books/book-05/cover.jpeg`. Then run the release workflow, inspect the thumbnail and rendered package, confirm byte-identical cover embedding, require EPUBCheck zero errors/warnings, create the deterministic release snapshot, and only then consider changing Book 5 to `upload_ready`.
